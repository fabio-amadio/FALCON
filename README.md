...
# Utils
Train with no residual actions:
...
num_envs=4096 \
project_name=g1_29dof_falcon \
experiment_name=g1_29dof_falcon_no_residual \
+opt=wandb \
obs.add_noise=True \
env.config.fix_upper_body_prob=0.3 \
robot.dof_effort_limit_scale=0.9 \
...
Train with hybrid pose/force tracking:

```bash
python humanoidverse/train_agent.py \
+exp=decoupled_locomotion_stand_height_waist_wbc_hybrid_force_tracking_ma_ppo_ma_env \
+simulator=isaacgym \
+domain_rand=domain_rand_rl_gym \
+terrain=terrain_locomotion_plane \
num_envs=4096 \
project_name=g1_29dof_falcon \
experiment_name=g1_29dof_falcon_hybrid_force_tracking \
+opt=wandb \
obs.add_noise=True \
robot.dof_effort_limit_scale=0.9 \
rewards.reward_initial_penalty_scale=0.1 \
...
This env already sets `env.config.fix_upper_body_prob=0.0` in its config, so the command above intentionally does not override it.

How it works:
- It alternates between pose mode and force mode at the normal command-resampling interval.
- In force mode, the actor receives force commands in a yaw-aligned body frame while palm pose references are neutralized.
- The external force is generated by a spring-damper field around a hidden palm reference and applied at the current palm position.

Train with hybrid pose/force tracking and true applied EE force exposed to the actor:

```bash
python humanoidverse/train_agent.py \
+exp=decoupled_locomotion_stand_height_waist_wbc_hybrid_force_tracking_true_force_ma_ppo_ma_env \
+simulator=isaacgym \
+domain_rand=domain_rand_rl_gym \
+terrain=terrain_locomotion_plane \
num_envs=4096 \
project_name=g1_29dof_falcon \
experiment_name=g1_29dof_falcon_hybrid_force_tracking_true_force \
+opt=wandb \
obs.add_noise=True \
robot.dof_effort_limit_scale=0.9 \
rewards.reward_initial_penalty_scale=0.1 \
...
This is the teacher-side simplification: both actor and critic receive the env-known applied EE force in the yaw-aligned body frame.
Reference: this variant mirrors the teacher/privileged side used in the original `learning-compliance` codebase, where the applied gripper force is part of the privileged signal and the student policy later learns an estimator from history.
