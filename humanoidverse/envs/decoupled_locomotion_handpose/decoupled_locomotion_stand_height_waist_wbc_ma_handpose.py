...
DEBUG = False
class LeggedRobotDecoupledLocomotionStanceHeightWBCHandPoses(LeggedRobotDecoupledLocomotionStance):
    ...
    def _init_tracking_config(self):
        ...
        self.left_hand_command_link = self.config.get("left_hand_command_link", "left_hand_link")
        self.right_hand_command_link = self.config.get("right_hand_command_link", "right_hand_link")
        self.left_hand_command_link_id = self.simulator._body_list.index(self.left_hand_command_link)
        self.right_hand_command_link_id = self.simulator._body_list.index(self.right_hand_command_link)
    ...
    def _pre_compute_observations_callback(self, debug=DEBUG):
        ...
        if self.config.rewards.fix_upper_body:
            ...
            if not self.fixed_hand_pose_initialized:
                self._init_fixed_hand_pose_local_from_fk()
            self.command_hand_pose_local[:] = self.fixed_hand_pose_local_template.unsqueeze(0)
            return
        ...
        self.ref_body_rot_extend[:, :motion_res["rg_rot_t"].shape[1], :] = motion_res["rg_rot_t"] # [num_envs, 4]
        ...
        # Hand-pose command and joint reference must stay coherent; do not scale the motion reference here.

        left_hand_pos_world = self.ref_body_pos_extend[:, self.left_hand_command_link_id, :]
        left_hand_quat_world = self.ref_body_rot_extend[:, self.left_hand_command_link_id, :]
        right_hand_pos_world = self.ref_body_pos_extend[:, self.right_hand_command_link_id, :]
        right_hand_quat_world = self.ref_body_rot_extend[:, self.right_hand_command_link_id, :]
        self.command_hand_pose_local[:] = self._compute_hand_pose_local(
            left_hand_pos_world, left_hand_quat_world, right_hand_pos_world, right_hand_quat_world
        )

    def _init_fixed_hand_pose_local_from_fk(self):
        if self._handpose_fk_parser is None:
            if self._motion_lib is not None and getattr(self._motion_lib, "mesh_parsers", None) is not None:
                self._handpose_fk_parser = self._motion_lib.mesh_parsers
            else:
                self._handpose_fk_parser = Humanoid_Batch(self.config.robot.motion, self.device)

        parser = self._handpose_fk_parser
        pose = torch.zeros(1, 1, parser.num_bodies_augment, 3, device=self.device)
        trans = torch.zeros(1, 1, 3, device=self.device)
        fk_res = parser.fk_batch(pose, trans, return_full=False)
        body_names = parser.body_names_augment
        torso_id = body_names.index(self.torso_name)
        left_id = body_names.index(self.left_hand_command_link)
        right_id = body_names.index(self.right_hand_command_link)

        torso_pos_world = fk_res.global_translation_extend[0, 0, torso_id].unsqueeze(0)
        torso_quat_world = fk_res.global_rotation_extend[0, 0, torso_id].unsqueeze(0)
        left_hand_pos_world = fk_res.global_translation_extend[0, 0, left_id].unsqueeze(0)
        left_hand_quat_world = fk_res.global_rotation_extend[0, 0, left_id].unsqueeze(0)
        right_hand_pos_world = fk_res.global_translation_extend[0, 0, right_id].unsqueeze(0)
        right_hand_quat_world = fk_res.global_rotation_extend[0, 0, right_id].unsqueeze(0)

        fixed_pose_local = self._compute_hand_pose_local(
            left_hand_pos_world, left_hand_quat_world,
            right_hand_pos_world, right_hand_quat_world,
            torso_pos_world=torso_pos_world,
            torso_quat_world=torso_quat_world,
        )
    ...
    def _compute_hand_pose_local(self, left_hand_pos_world, left_hand_quat_world, right_hand_pos_world, right_hand_quat_world, torso_pos_world=None, torso_quat_world=None):
        ...
        if torso_quat_world is None:
            torso_quat_world = self.simulator._rigid_body_rot[:, self.torso_index, :4]
        torso_quat_inv = quat_conjugate(torso_quat_world)

        left_hand_pos_local = quat_rotate_inverse(torso_quat_world, left_hand_pos_world - torso_pos_world)
        right_hand_pos_local = quat_rotate_inverse(torso_quat_world, right_hand_pos_world - torso_pos_world)
        left_hand_quat_local = quat_mul(torso_quat_inv, left_hand_quat_world)
        right_hand_quat_local = quat_mul(torso_quat_inv, right_hand_quat_world)
        left_hand_quat_local = left_hand_quat_local / torch.norm(left_hand_quat_local, dim=-1, keepdim=True).clamp_min(1e-6)
        right_hand_quat_local = right_hand_quat_local / torch.norm(right_hand_quat_local, dim=-1, keepdim=True).clamp_min(1e-6)

        batch_size = left_hand_pos_world.shape[0]
        hand_pose_local = torch.zeros(batch_size, 14, dtype=torch.float32, device=self.device, requires_grad=False)
        hand_pose_local[:, 0:3] = left_hand_pos_local
        hand_pose_local[:, 3:7] = left_hand_quat_local
        hand_pose_local[:, 7:10] = right_hand_pos_local
    ...
    def reset_envs_idx(self, env_ids, target_states=None, target_buf=None):
        ...
        # Disable upper-body reference scaling in hand-pose env to avoid pose/reference mismatch.
    ...
    def _reward_tracking_command_hand_pose(self):
        ...
        cmd_hand_pose_local = self.command_hand_pose_local

        pos_error = torch.sum(torch.square(curr_hand_pose_local[:, 0:3] - cmd_hand_pose_local[:, 0:3]), dim=1) + \
                    torch.sum(torch.square(curr_hand_pose_local[:, 7:10] - cmd_hand_pose_local[:, 7:10]), dim=1)

        left_quat_dot = torch.sum(curr_hand_pose_local[:, 3:7] * cmd_hand_pose_local[:, 3:7], dim=1).abs().clamp(max=1.0)
        right_quat_dot = torch.sum(curr_hand_pose_local[:, 10:14] * cmd_hand_pose_local[:, 10:14], dim=1).abs().clamp(max=1.0)
        rot_error = (1.0 - left_quat_dot * left_quat_dot) + (1.0 - right_quat_dot * right_quat_dot)

        return torch.exp(
            -pos_error / self.config.rewards.reward_tracking_sigma.hand_pose_pos
            -rot_error / self.config.rewards.reward_tracking_sigma.hand_pose_rot
        )
    ...
    def _get_obs_command_hand_pose_local(self):
        return self.command_hand_pose_local
